...
@activo bit,@minimo decimal,@maximo decimal,@descripcion varchar(950),@sello bit,
@descripcionAbreviada varchar(50),@referencia varchar(250),@uMedidaCompra varchar(10),
@uMedidaConsumo varchar(10),@papeleta varchar(50),@notas varchar(1550),@orden int,@descuento bit,
@usuarioActualiza varchar(50),@grupoIR varchar(50),@tipo varchar(2),@Retorno int output 
 
AS begin tran iItems 
update iItems set tiempoReposicion = @tiempoReposicion,foto = @foto,
...
papeleta = @papeleta,notas = @notas,usuarioActualiza = @usuarioActualiza,grupoIR = @grupoIR,tipo = @tipo , sello=@sello,
descuento=@descuento
where empresa = @empresa and codigo = @codigo 

if (@@error = 0 ) begin set @Retorno = 0 commit tran iItems end else begin set @Retorno = 1 
