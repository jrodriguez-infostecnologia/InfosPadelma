...
declare @tercero varchar(50),@banco varchar(50),@cadena varchar(max),
		...
		begin	
		if exists( select *  from nPlanoBancoDetalle
		...
		close curPlano
		deallocate curPlano
		set @cadena = substring(@cadena,1,len(@cadena)-1) + '  from vSeleccionaPago where cheque=0 and empresa= ' + @empresa +' and noPeriodo='+@periodo +' and año='+@año +' and anulado=0 and numero like  ' + char(39) + '%'+@numero+'%' + char(39) + ' and tercero ='+ @tercero
		EXECUTE( @cadena )
		...
		--select top 10 * from vSeleccionaPago
		--where empresa=1