...
public partial class Laboratorio_Panalisis_ImprimeRemision : System.Web.UI.Page
{
    ...
    private void imprimeRemision()
    {
        ...
        if (Convert.ToString(this.rblTipoImpresion.SelectedValue) == "RINV")
            tipo = "RemisionInversora";
        else
            tipo = "RemisionPadelma";

        switch (vehiculos.InsertaDespacho(Convert.ToString(this.ddlRemision.SelectedValue), Convert.ToString(this.rblTipoImpresion.SelectedValue), Convert.ToInt16(Session["empresa"])))
        {
            case 1:
