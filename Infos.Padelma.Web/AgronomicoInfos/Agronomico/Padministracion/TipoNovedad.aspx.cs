...
public partial class Agronomico_Padministracion_TipoNovedad : System.Web.UI.Page
{
    ...
    private void GetEntidad()
    {
        try
        {
            ...
            this.gvLista.DataSource = novedades.BuscarEntidadTipo(
                Convert.ToString(nitxtBusqueda.Text), Convert.ToInt32(Session["empresa"]));
    ...
    private void CargarCombos()
    {
        ...
        try
        {
            this.ddlTipoTransaccion.DataSource = transacciones.GetTipoTransaccionModulo(Convert.ToInt32(this.Session["empresa"]));
        ...
        try
        {
            ...
            using (TransactionScope ts = new TransactionScope())
            {
                ...
                if (Convert.ToBoolean(this.Session["editar"]) == true)
                {
                    for (int x = 0; x < this.selNovedades.Items.Count; x++)
                    {
                        if (novedades.VerificaNovedadTipo(ddlTipoTransaccion.SelectedValue, Convert.ToInt32(this.selNovedades.Items[x].Value), Convert.ToInt32(Session["empresa"])) == 0)
                        {
                            if (this.selNovedades.Items[x].Selected == false)
                            {
                                ...
                                object[] objValoresConcepto = new object[]{
                                                        ...
                                                        Convert.ToInt32(this.selNovedades.Items[x].Value),
    ...
    private void ValidaRegistro()
    {
        for (int x = 0; x < this.selNovedades.Items.Count; x++)
        {
            if (novedades.VerificaNovedadTipo(ddlTipoTransaccion.SelectedValue, Convert.ToInt32(selNovedades.Items[x].Value), Convert.ToInt32(Session["empresa"])) == 0)
    ...
    protected void gvLista_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        try
        {
            ...
            if (novedades.EliminaNovedades(Convert.ToString(this.gvLista.Rows[e.RowIndex].Cells[2].Text), Convert.ToInt32(Session["empresa"])) == 0)
            {
