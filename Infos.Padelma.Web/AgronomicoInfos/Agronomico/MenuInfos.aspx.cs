...
public partial class Infos_MenuInfos : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            try
            {
                ...
                this.hpMenuPrincipal.Attributes.Add("OnClick", "javascript: document.location.href='" + menu.SeleccionaMenuPrincipal(this.Session["usuario"].ToString(), this.Session["clave"].ToString(), Convert.ToInt32(this.Session["empresa"])).ToString() + "'");
                ...
                if (Session["empresa"] == null)
                {
                    ...
                    this.lbEmpresa.Text = menu.RetornaNombreEmpresa(Convert.ToInt32(this.Session["empresa"].ToString()));
                    lbIdEmpresa.Text = Session["empresa"].ToString();
                }
                else
                {
                    this.lbEmpresa.Text = menu.RetornaNombreEmpresa(Convert.ToInt32(this.Session["empresa"]));
                    lbIdEmpresa.Text = Session["empresa"].ToString();
    ...
    protected void gvLista_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            seguridad.InsertaLog(Convert.ToString(Session["usuario"]), "IN", ConfigurationManager.AppSettings["Modulo"].ToString() + '-' + this.Page.ToString(),
                    "ex", "Cambio de empresa", ObtenerIP(), Convert.ToInt32(this.Session["empresa"].ToString()));
        }
        catch (Exception ex)
        {
        }
    ...
    {
        Response.Redirect(menu.SeleccionaMenuPrincipal(this.Session["usuario"].ToString(), this.Session["clave"].ToString(), Convert.ToInt32(this.Session["empresa"])));
    }
    #endregion Metodos
}