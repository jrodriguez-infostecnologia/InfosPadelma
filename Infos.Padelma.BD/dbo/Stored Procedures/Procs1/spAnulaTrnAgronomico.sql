...
*****************************************************************************/
	...
	begin tran Anulacion
		...
		set @referencia= isnull(@referencia,'')
			...
			-- actualiza saldos de items
		
		exec spActualizaSaldoItemTransaccion @empresa , @tipo, @numero, @retorno

		if not exists (select * from aTransaccionItem where empresa=@empresa and numero=@referencia  and saldo<>cantidad)
	begin
	update aTransaccion
	set cerrado=0
	where empresa=@empresa and numero=@referencia
	end
