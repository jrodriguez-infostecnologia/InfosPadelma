...
@uMedida varchar(50),@seccion varchar(50),@precioLabor money,@registroNovedad int,
...
set @perido = (select periodo from aTransaccion where numero=@numero and tipo=@tipo and empresa=@empresa)
 ...
 insert aTransaccionNovedad( fecha,empresa,año,mes,novedad,registro,racimos,pesoRacimo,ejecutado,cantidad,jornales,saldo,tipo,numero,uMedida,seccion,lote,signo, precioLabor,periodo,concepto,registroNovedad ) 
 select @fecha,@empresa,@año,@mes,@novedad,@registro, @recimos,@pesoRacimo,@ejecutado,@cantidad,@jornales,@saldo,@tipo,@numero,@uMedida,@seccion,@lote,@signo ,@precioLabor,@perido,@concepto,@registroNovedad
