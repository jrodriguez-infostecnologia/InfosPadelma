CREATE proc [dbo].[spSelecionaInformacionCertificadoIngresoRetenciones]
 @empresa int, @año  int
 as
...
declare @conceptoIncapacidad varchar(50) =  (select incapacidades from nParametrosGeneral where empresa=@empresa) -- <--joe me dijo que sumaba solo lo real de la incapacidad ricardo gomez 03/05/2017
...
sum( case when b.concepto <> @conceptoIncapacidad then  b.valorTotal else b.valorTotalR end ) valorTotal,isnull(h.codigo,'999') tipoConcepto,isnull(h.descripcion,'Otros') nombreTipoConcepto
...
left join nConcepto i on b.concepto=i.codigo and b.empresa=i.empresa
where a.empresa=@empresa and a.año=@año and a.anulado=0 --and c.codigo='19614823'
