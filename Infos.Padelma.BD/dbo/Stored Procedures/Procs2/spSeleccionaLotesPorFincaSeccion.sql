...
as
	...
	begin
		...
		isnull(b.codigo,'') codSeccion, b.descripcion nombreSeccion, b.hBrutas HaSeccion, c.codigo codLote, c.descripcion nombreLote, convert(varchar(4),añoSiembra) + '-'+  rtrim(RIGHT('00' + rtrim(mesSiembra), 2))   siembra,palmasBrutas,palmasProduccion,c.hBrutas haLotes ,hNetas,dSiembra,
		densidad,e.descripcion nombreVariedad , e.procedencia procedenciaVariedad , isnull(sum(g.noPalma),c.palmasProduccion) noPalmas,
		c.activo estado,convert(varchar,añoSiembra) añoSiembra ,convert(varchar,c.variedad) variedad
		from aFinca a
		...
		densidad,e.descripcion  , e.procedencia,c.activo ,c.variedad
	end
	...
	begin
		...
		c.activo estado ,convert(varchar,añoSiembra) añoSiembra ,convert(varchar,c.variedad) variedad
		from aFinca a
		left join aLotes c on c.finca=a.codigo  and c.empresa=a.empresa and c.activo=1
		left join aSecciones b on b.finca=a.codigo and c.seccion=b.codigo and b.empresa=a.empresa
		left join gCiudad d on d.codigo=a.ciudad and d.empresa=a.empresa
		left join aVariedad e on e.codigo=c.variedad and e.empresa=c.empresa
		left join cTercero f on f.id=a.proveedor and f.empresa=a.empresa
	...
	begin
		...
		c.activo estado ,convert(varchar,añoSiembra) añoSiembra,convert(varchar,c.variedad) variedad
		from aFinca a
		left join aLotes c on c.finca=a.codigo  and c.empresa=a.empresa and c.activo=0
		left join aSecciones b on b.finca=a.codigo and c.seccion=b.codigo and b.empresa=a.empresa
		left join gCiudad d on d.codigo=a.ciudad and d.empresa=a.empresa
		left join aVariedad e on e.codigo=c.variedad and e.empresa=c.empresa
		left join cTercero f on f.id=a.proveedor and f.empresa=a.empresa
