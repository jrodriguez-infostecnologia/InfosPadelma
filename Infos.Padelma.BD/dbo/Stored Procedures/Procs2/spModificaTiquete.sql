...
*****************************************************************************/
	...
	if exists (select * from gParametrosGenerales where empresa=@empresa  and @tipo in (salidas,salidasAlt)		)
	begin
		...
		tercero=@tercero,
		cliente =@cliente
		where empresa=@empresa and numero in 
		(select remision from bRegistroBascula where tipo=@tipo and numero=@numero)
