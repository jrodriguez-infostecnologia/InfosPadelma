...
select distinct a.id,a.nombre, a.codigo idGrupo, a.descripcion grupo, isnull(b.contratista,0) contratista, isnull(desarrollo,0) desarrollo,isnull(b.valorTotal,0) valorTotal  
from (select * from @mes, aGrupoNovedad where aGrupoNovedad.empresa=@empresa) a  left join (
...
) b on a.codigo=b.codLabor and a.id = b.mes and a.empresa=@empresa