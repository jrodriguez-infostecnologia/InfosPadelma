...
where empresa=@empresa
	...
	begin
		...
		+ isnull((select sum(case when cantidad <0 then -1 *  cantidad else cantidad end) from vValorAcumuladoPrimas where empresa=@empresa and tercero=@tercero  and ausentismo=1 and año=YEAR(@fechaFinalPrima) AND noPeriodo between @periodoInicial and  @periodoFinal  and contrato=@contrato),0)
...
e.codigo ccostoMayor,e.descripcion nombreccostoMayor from tmpLiquidacionPrima a
