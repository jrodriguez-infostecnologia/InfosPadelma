...
YEAR(a.fecha) año,month(a.fecha) mes,dbo.fRetornaNombreMes(month(a.fecha)) nombreMes,a.variedad,a.seccion,a.nombreVariedad
from vSeleccionLaboresProuduccion a
where a.empresa=@empresa and a.claseLabor=2 and year(a.fecha)=@año and a.anulado=0 
group by  añoSiembra,codLote,nombreLote,palmasProduccion,hNetas,year(fecha),MONTH(fecha),a.variedad,a.seccion,a.nombreVariedad