...
select  @conceptoVacacion=vacaciones
 ...
 declare cursorFuncionarios insensitive cursor for	
	select distinct  a.tercero,b.fechaIngreso,max(b.id),b.auxilioTransporte,
	case when b.salarioAnterior=0 then b.salario else  b.salarioAnterior end
	from nFuncionario  a
	...
	where  a.empresa=@empresa and b.activo=1  and b.fechaIngreso<=@fechaCorte
	...
	begin	
			...
			where empleado=@tercero and empresa=@empresa and anulado=0 and contrato=@contrato 
			and fechaSalida <=@fechaCorte
