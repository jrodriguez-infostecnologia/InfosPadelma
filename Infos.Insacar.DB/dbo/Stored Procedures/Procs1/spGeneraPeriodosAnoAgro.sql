CREATE PROCEDURE [dbo].[spGeneraPeriodosAnoAgro]
	@ano		int,
	@empresa    int,
	@retorno	int output,
	@conteo		int output
...
declare @fecha varchar(8)
	...
	begin tran Inserta
		...
		begin
			...
			if( not exists ( select top 1 * from aPeriodo
			...
			set @descripcion = DATENAME(month,convert(date,(cast(@ano as char(4))+'/'+cast(@mes as varchar(50))+'/01'))) 
