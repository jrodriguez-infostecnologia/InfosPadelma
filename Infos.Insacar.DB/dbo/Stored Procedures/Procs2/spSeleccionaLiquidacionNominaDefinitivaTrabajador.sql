CREATE proc [dbo].[spSeleccionaLiquidacionNominaDefinitivaTrabajador]
@empresa int,
@año int,
@periodo int,
@cedula varchar(50)
...
select   a.identificacion, a.codTercero, a.nombreTercero, a.codCCosto, a.nombreCcosto, a.sueldo, a.nombreCargo, a.codConcepto,
 a.nombreConcepto, case when mostrarCantidad=1 then NULL else  
case when a.noMostrar=1 then a.cantidadR else a.cantidad end end cantidad,a.valorTotal valorConcepto, 
...
and a.codConcepto not in (select distinct isnull(concepto,'') from aNovedad  where empresa=@empresa )
and a.noPeriodo=@periodo and a.año=@año and a.anulado=0 and a.codiTercero=@cedula
and a.tipo='LQN'
...
and convert(date,b.fecha) between @fechaInicial and @fechaFinal and  a.codiTercero=@cedula
and a.tipo='LQN'
