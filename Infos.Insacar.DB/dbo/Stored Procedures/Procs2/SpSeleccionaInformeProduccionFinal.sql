CREATE proc [dbo].[SpSeleccionaInformeProduccionFinal]
@periodo varchar(50),
@empresa	int
as
...
where month(a.fecha)=substring(@periodo,5,LEN(@periodo))
and YEAR(a.fecha)=substring(@periodo,1,4)
...
from vTransaccionesProduccion a
	...
	join iItems d on d.codigo=a.movimiento and d.tipo='M' and d.empresa=a.empresa
	join iItems c on c.codigo=a.producto and c.tipo='P' and c.empresa=a.empresa
where 	YEAR( a.fecha ) = SUBSTRING(@periodo,1,4)	and MONTH(a.fecha)=SUBSTRING(@periodo,5,LEN(@periodo))
	and b.mInforme=1 and a.empresa=a.empresa and a.anulado=0
group by c.descripcion,d.descripcion,a.movimiento,refMovimiento,c.orden,a.producto,b.orden,DATENAME(MONTH,fecha),DATEPART(MONTH,fecha),a.empresa, a.almacena